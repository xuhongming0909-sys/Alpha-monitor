...
│  ├─ lof_iopv/  QDII LOF IOPV 估值（指数型+主动型）                       │
│  └─ ... (共 10 个插件)                                   │
└──────────────────────────────────────────────────────────┘
    │
    ▼
...
| `lof_iopv/` | 东财+Yahoo 5m(美股/期货)+腾讯(A股/港股)+Sina(国内期货) | `fetcher.py`, `source.py`, `fund_classifier.py`, `normalizer.py` | QDII LOF IOPV二分类(指数/主动)数据 |
...
| `src/components/LofCardList.jsx` | LOF IOPV估值表格：二分类展示(指数型/主动型)+MAE列 |
...
| data_fetch/lof_iopv/source.py | LOF IOPV数据获取层（东财净值+Yahoo 5m全球价格+腾讯A股/港股+Sina国内期货+navDatePrices Yahoo兜底） |
...
| `data_fetch/lof_iopv/fund_classifier.py` | LOF二分类(指数型/主动型)基金分类器+持仓获取 |
...
| `strategy/lof_iopv/calc.py` | 共享IOPV计算公式：指数型(ETF映射)+主动型(T10持仓加权) |
| `strategy/lof_iopv/service.py` | QDII LOF IOPV业务计算：二分类持仓来源+响应构建+监控池筛选 |
