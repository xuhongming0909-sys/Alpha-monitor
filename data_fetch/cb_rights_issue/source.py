...
MA_WINDOW = max(1, int(_STRATEGY_CONFIG.get("moving_average_window") or 20))
...
def _calc_ma_from_closes(closes: List[float], window: int) -> Optional[float]:
...
def _load_history_metrics(stock_codes: Iterable[str]) -> Dict[str, Dict[str, Any]]:
    ...
    for stock_code in sorted({normalize_stock_code(code) for code in stock_codes if normalize_stock_code(code)}):
        closes = history_db.load_recent_closes(stock_code, max(VOL_WINDOW + 1, MA_WINDOW))
        metrics[stock_code] = {
            "closeCount": len(closes),
            "volatility60": _calc_volatility_from_closes(closes),
            "ma20Close": _calc_ma_from_closes(closes, MA_WINDOW),
        }
    return metrics
...
def get_cb_rights_issue_source_snapshot() -> Dict[str, Any]:
    ...
    for item in raw_rows:
        ...
        rows.append({
            ...
            "ma20Price": to_float(item.get("ma20_price")),
            "ma20CloseDb": history_info.get("ma20Close"),
