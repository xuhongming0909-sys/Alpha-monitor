...
if __name__ == "__main__":
    ...
    if action == "dividend" and len(sys.argv) >= 3:
        ...
        print(json.dumps(get_dividend_data(stock_code), ensure_ascii=False, default=str))
        sys.exit(0)

    if action == "upcoming":
        days = int(sys.argv[2]) if len(sys.argv) >= 3 else 3
        print(json.dumps(get_upcoming_dividends(days), ensure_ascii=False, default=str))
        sys.exit(0)

    if action == "price" and len(sys.argv) >= 3:
        stock_code = sys.argv[2]
        print(json.dumps(get_stock_price(stock_code), ensure_ascii=False, default=str))
        sys.exit(0)
