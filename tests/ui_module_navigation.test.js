...
// Required tab labels
const tabs = ['转债', 'AH', 'AB', 'LOF', '打新', '监控', '分红'];
for (const tab of tabs) {
  assert.ok(app.includes(tab), `App should include ${tab} tab`);
}

// Module table components
assert.ok(/AhTable|AbTable|LofTable|SubscriptionTable|MonitorTable|DividendTable/i.test(app), 'App should have module table components');
