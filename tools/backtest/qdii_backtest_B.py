# AI-SUMMARY: QDII B类T10持仓法回测脚本（旧版）
...
def get_fx_history():
    """姹囩巼鍘嗗彶 - akshare澶涓棿浠?(涓浗澶栨眹浜ゆ槗涓績)"""
    import akshare as ak
    fx = {}
    # USD/CNY
    try:
        df_usd = ak.currency_boc_sina(symbol="缇庡厓", start_date="20260301", end_date="20260526")
        for _, row in df_usd.iterrows():
            fx[str(row['鏃ユ湡'])] = {'usd': float(row['澶涓棿浠?]) / 100}
    except Exception as e:
        print(f'  [!] USD FX error: {e}')
    # HKD/CNY
    try:
        df_hkd = ak.currency_boc_sina(symbol="娓竵", start_date="20260301", end_date="20260526")
        for _, row in df_hkd.iterrows():
            d = str(row['鏃ユ湡'])
            if d not in fx:
                fx[d] = {}
            fx[d]['hkd'] = float(row['澶涓棿浠?]) / 100
    except Exception as e:
...
log('=' * 95)
log('B绫? 鍓嶅崄澶ф寔浠撲及鍊?(3鍙? 鍏ㄩ儴鏁版嵁浠庡浗鍐匒PI鑾峰彇)')
log('鎸佷粨: 涓滆储F10 API | 浠撲綅: 涓滆储pingzhongdata API | 鍑€鍊? 涓滆储API')
log('鑲′环: 鑵捐璐㈢粡K绾緼PI | 姹囩巼: akshare澶涓棿浠?)
log('鍏紡: est = (stock_ratio/100) * 危(ret_local 脳 (1+fx) 脳 weight) / 危(weight)')
log('瀵归綈: fund_prev == all tickers prev == FX date')
log('=' * 95)

# 1. Fetch FX data
print('\n鑾峰彇姹囩巼鏁版嵁...')
fx_data = get_fx_history()
log(f'姹囩巼鏁版嵁: {len(fx_data)} 澶?)
fx_dates = sorted(fx_data.keys())
if fx_dates:
    log(f'  鍖洪棿: {fx_dates[0]} ~ {fx_dates[-1]}')
    # Show a sample
    sample_d = fx_dates[0]
    fx_sample = fx_data[sample_d]
    log(f'  鏍蜂緥 {sample_d}: USD/CNY={fx_sample.get("usd","N/A")}, HKD/CNY={fx_sample.get("hkd","N/A")}')
...
for code in sorted(B_FUNDS.keys()):
    ...
    # 6. Alignment
    # Common dates: fund_ret 鈭?all ticker prices 鈭?fx_ret
...
log(f'\n鏁版嵁婧? 鍑€鍊?涓滆储API, 鎸佷粨=涓滆储F10, 浠撲綅=涓滆储pingzhongdata, 鑲′环=鑵捐K绾? 姹囩巼=akshare澶涓棿浠?)
