// AI-SUMMARY: AH溢价全局卡片列表视图
// 对应 INDEX.md §9.3 文件摘要索引

import React from 'react';

const PERCENT_FORMAT = new Intl.NumberFormat('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

function toNumber(value) {
  if (value === null || value === undefined) return null;
  const number = Number(value);
  return Number.isFinite(number) ? number : null;
}

function formatNumber(value, suffix = '') {
  const number = toNumber(value);
  return number === null ? '--' : `${number.toLocaleString('zh-CN', { maximumFractionDigits: 2 })}${suffix}`;
}

function formatPercent(value) {
  const number = toNumber(value);
  return number === null ? '--' : `${PERCENT_FORMAT.format(number)}%`;
}

function signedClass(value) {
  const number = toNumber(value);
  if (number === null || number === 0) return 'is-flat';
  return number > 0 ? 'is-up' : 'is-down';
}

function pickText(...values) {
  for (const value of values) {
    const text = String(value ?? '').trim();
    if (text) return text;
  }
  return '--';
}

function rowMatchesQuery(row, query, fields) {
  if (!query) return true;
  const lower = query.toLowerCase();
  return fields.some((f) => String(row[f] || '').toLowerCase().includes(lower));
}

function AhCard({ row, rank }) {
  const [expanded, setExpanded] = React.useState(false);
  const aPrice = toNumber(row.aPrice);
  const hPriceCny = toNumber(row.hPriceCny);
  const gap = (aPrice !== null && hPriceCny !== null) ? (aPrice - hPriceCny).toFixed(2) : null;

  return (
    <div className="ah-card" style={{
      padding: '12px',
      borderBottom: '1px solid var(--terminal-line-soft)',
      background: 'var(--terminal-panel)',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: '15px', fontWeight: 700, lineHeight: 1.3 }}>
            {pickText(row.aName, row.name)}
          </div>
          <div style={{ fontSize: '12px', color: 'var(--terminal-muted)', fontFamily: 'var(--terminal-mono)', marginTop: '2px' }}>
            A:{pickText(row.aCode, row.code)} · H:{pickText(row.hCode)}
          </div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: '12px', color: 'var(--terminal-faint)' }}>溢价率</div>
          <div className={`mono ${signedClass(row.premium)}`} style={{ fontSize: '18px', fontWeight: 700 }}>
            {formatPercent(row.premium)}
          </div>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '8px' }}>
        <div>
          <div style={{ fontSize: '10px', color: 'var(--terminal-faint)', marginBottom: '2px' }}>A股价格</div>
          <div className="mono" style={{ fontSize: '14px', fontWeight: 700 }}>{formatNumber(row.aPrice)}</div>
        </div>
        <div>
          <div style={{ fontSize: '10px', color: 'var(--terminal-faint)', marginBottom: '2px' }}>H股人民币</div>
          <div className="mono" style={{ fontSize: '14px', fontWeight: 700 }}>{formatNumber(row.hPriceCny)}</div>
        </div>
        <div>
          <div style={{ fontSize: '10px', color: 'var(--terminal-faint)', marginBottom: '2px' }}>近三年分位</div>
          <div className="mono" style={{ fontSize: '14px', fontWeight: 700 }}>{formatNumber(row.percentile)}</div>
        </div>
      </div>

      {expanded && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 12px', padding: '8px 0', borderTop: '1px solid var(--terminal-line-soft)', fontSize: '12px' }}>
          <div><span style={{ color: 'var(--terminal-faint)' }}>H股价格: </span><span className="mono">{formatNumber(row.hPrice)}</span></div>
          <div><span style={{ color: 'var(--terminal-faint)' }}>价差: </span><span className={`mono ${signedClass(gap)}`}>{gap !== null ? (Number(gap) > 0 ? '+' : '') + gap : '--'}</span></div>
          <div><span style={{ color: 'var(--terminal-faint)' }}>样本数: </span><span className="mono">{formatNumber(row.historyCount)}</span></div>
          <div><span style={{ color: 'var(--terminal-faint)' }}>样本区间: </span><span className="mono">{pickText(row.historyStartDate, row.historyEndDate) ? `${row.historyStartDate?.slice(0,10) || ''}~${row.historyEndDate?.slice(0,10) || ''}` : '--'}</span></div>
        </div>
      )}

      <button
        onClick={() => setExpanded((v) => !v)}
        style={{
          width: '100%',
          padding: '6px',
          border: 0,
          background: 'transparent',
          color: 'var(--terminal-faint)',
          fontSize: '12px',
          cursor: 'pointer',
          marginTop: '4px',
        }}
      >
        {expanded ? '▲ 收起' : '▼ 展开'}
      </button>
    </div>
  );
}

export default function AhCardList({ rows, searchQuery }) {
  const filtered = rows.filter((r) => rowMatchesQuery(r, searchQuery, ['aName', 'name', 'aCode', 'code', 'hName', 'hCode']));

  return (
    <section className="terminal-panel main-table-panel">
      <div className="panel-head compact-head">
        <div><p className="eyebrow">AH PREMIUM</p><h2>AH 溢价</h2></div>
        <span className="panel-count">{filtered.length} 条</span>
      </div>
      <div style={{ maxHeight: 'calc(100vh - 220px)', overflowY: 'auto' }}>
        {filtered.length ? filtered.map((row, index) => (
          <AhCard key={`${row.aCode || row.code || index}`} row={row} rank={index + 1} />
        )) : (
          <div className="empty-cell" style={{ padding: '24px 12px' }}>暂无数据</div>
        )}
      </div>
    </section>
  );
}
